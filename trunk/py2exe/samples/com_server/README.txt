...
Python 2.2 or build 155 for Python 2.3, or later.

The COM server will be the 'Python Interpreter' COM server sample
included in the win32all extensions.

Run the supplied 'setup_interp.py py2exe', appending either '--com-dll'
(to build an inprocess server) , or '--com-exe' (to build a localserver).
...
interp.dll. There are several ways to do this, with the most common
being to run "regsvr32.exe dll_name.dll".  If you need more information,
ask google ;-)
